...
def generate_1d_grating(
    ...
    lib = _new_library()
    unit = lib.new_cell("UNIT_1D_LINE")
    unit.add(rectangle_polygon(line_width, total_width_um, layer=layer))

    main = lib.new_cell("MAIN")
    main.add(_cell_array(unit, columns=columns, rows=1, spacing=(period_um, 0), origin=(line_width / 2.0, total_width_um / 2.0)))
...
def generate_2d_periodic(
    ...
    lib = _new_library()
    unit = lib.new_cell(f"UNIT_{shape.upper()}")

    if shape == "circle":
        diameter = positive_float(diameter_um or 0, "Diameter")
        unit.add(circle_polygon(diameter, points=circle_points, layer=layer))
        size_summary = {"diameter_um": diameter}
    elif shape == "square":
        side = positive_float(square_side_um or 0, "Square side")
        unit.add(rectangle_polygon(side, side, layer=layer))
        ...
        unit.add(rectangle_polygon(length, width, layer=layer))
