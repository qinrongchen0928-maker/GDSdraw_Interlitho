...
def _draw_1d_grating(draw: ImageDraw.ImageDraw, period: float, duty: float, total_l: float, total_w: float, scale: float) -> None:
    columns = floor(total_l / period)
    line_w = period * duty
    for col in range(columns):
        x0 = col * period * scale
        x1 = (col * period + line_w) * scale
        draw.rectangle((x0, 0, x1, total_w * scale), fill=INK)
    ...
    for row in range(0, rows, step):
        ...
        for col in range(0, cols, step):
            ...
            if shape == "circle":
                ...
                draw.ellipse((cx - d / 2, cy - d / 2, cx + d / 2, cy + d / 2), fill=INK)
            elif shape == "square":
                s = float(square_side or 0) * scale
                draw.rectangle((cx - s / 2, cy - s / 2, cx + s / 2, cy + s / 2), fill=INK)
            else:
...
def periodic_overview_1d(period: float, duty: float, total_l: float, total_w: float) -> bytes:
    width_px, height_px, scale = _canvas_size(total_l, total_w)
    image = Image.new("RGB", (width_px, height_px), BG)
    draw = ImageDraw.Draw(image)
    _draw_1d_grating(draw, period, duty, total_l, total_w, scale)
    _draw_frame(draw, width_px, height_px)
    return _to_bytes(image)


def periodic_detail_1d(period: float, duty: float, total_w: float) -> bytes:
    width_um = 3 * period
    ...
    _draw_1d_grating(draw, period, duty, width_um, height_um, scale)
    _draw_frame(draw, width_px, height_px)
    return _to_bytes(image)
...
def periodic_overview_2d(
    ...
    image = Image.new("RGB", (width_px, height_px), BG)
    draw = ImageDraw.Draw(image)
    _draw_2d_shape(draw, shape, period_x, period_y, total_l, total_w, scale, diameter, square_side, rect_l, rect_w)
    _draw_frame(draw, width_px, height_px)
    return _to_bytes(image)
...
def periodic_detail_2d(
    ...
    _draw_2d_shape(draw, shape, period_x, period_y, width_um, height_um, scale, diameter, square_side, rect_l, rect_w)
    _draw_frame(draw, width_px, height_px)
    return _to_bytes(image)
